...
```bash
npx playwright open --load-storage=_opencrew/_browser_profile/instagram.json https://www.instagram.com/{username}/
```
...
5. Download the audio for transcription using yt-dlp:
   ```bash
   yt-dlp -x --audio-format wav -o "crews/{crew-name}/_investigations/{username}/reel-{id}.%(ext)s" "https://www.instagram.com/reel/{reel-id}/"
   ```

6. Transcribe the audio using whisper:
   ```bash
   whisper "crews/{crew-name}/_investigations/{username}/reel-{id}.wav" --model small --output_dir "crews/{crew-name}/_investigations/{username}/" --output_format txt
   ```
