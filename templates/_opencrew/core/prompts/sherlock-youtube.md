...
1. Navigate to the channel's videos page using browser automation with the saved YouTube session:
   ```bash
   npx playwright open --load-storage=_opencrew/_browser_profile/youtube.json https://www.youtube.com/@{handle}/videos
   ```
...
```bash
yt-dlp --write-auto-sub --sub-lang en --skip-download -o "crews/{crew-name}/_investigations/{channel}/%(id)s" "https://www.youtube.com/watch?v={video-id}"
```
...
```bash
yt-dlp -x --audio-format wav -o "crews/{crew-name}/_investigations/{channel}/%(id)s.%(ext)s" "https://www.youtube.com/watch?v={video-id}"
```

```bash
whisper "crews/{crew-name}/_investigations/{channel}/{video-id}.wav" --model small --output_dir "crews/{crew-name}/_investigations/{channel}/" --output_format txt
```
