...
When a user provides reference profile URLs during crew discovery ("follow the style of @username"), the Architect dispatches one Sherlock subagent per profile URL. Each subagent navigates to the profile using browser automation, extracts real content (captions, text, slide content, video transcriptions), and produces a structured analysis of patterns found.

The investigation output feeds directly into crew data files — making agents, frameworks, quality criteria, and voice guidance grounded in real high-performing content rather than generic best practices.

## Session Management (ALWAYS inform the user)

At the START of every investigation, before any browser action, tell the user:

> "Your browser sessions are saved in `_opencrew/_browser_profile/`. To clear a platform's session, just delete the JSON file (e.g., `instagram.json`). I'll ask before saving any new session."
...
Browser sessions are stored as JSON files in `_opencrew/_browser_profile/`:

- Instagram: `_opencrew/_browser_profile/instagram.json`
- YouTube: `_opencrew/_browser_profile/youtube.json`
- Twitter/X: `_opencrew/_browser_profile/twitter.json`
- LinkedIn: `_opencrew/_browser_profile/linkedin.json`
...
```
crews/{crew-name}/_investigations/{username}/screenshots/{filename}.png
```
...
The `{crew-name}` value is always provided by the Architect in the Sherlock subagent prompt.
...
Session files are stored in `_opencrew/_browser_profile/` as JSON files (one per platform). This directory is gitignored and persists between runs.
...
7. If the user declines saving, inform them:
   "No problem. Remember your cookies are still in `_opencrew/_browser_profile/` — delete the folder to clear everything."
...
Users can clear all saved browser sessions at any time by deleting the `_opencrew/_browser_profile/` directory or individual JSON files. After deletion, the next investigation will require a fresh manual login.
...
- If extraction fails for any reason, save an `error.md` file to `crews/{crew-name}/_investigations/{username}/error.md` explaining what failed and why.
...
One file per profile, saved to `crews/{crew-name}/_investigations/{username}/raw-content.md`.
...
One file per profile, saved to `crews/{crew-name}/_investigations/{username}/pattern-analysis.md`.
...
Five specific, actionable recommendations for how the crew should incorporate
...
One file per investigation (across all profiles), saved to `crews/{crew-name}/_investigations/consolidated-analysis.md`.
...
This framework becomes the operational blueprint for the crew.
...
content. These become "never do" rules for the crew:
...
After the investigation completes, the Architect maps findings to crew data files during Phase 3 (Extraction). Each crew data file is enriched with real-world evidence from the investigation.

### Mapping: Investigation Output to Crew Data Files

| Investigation Section | Crew Data File | What Gets Extracted |
|---|---|---|
...
- **Ideator agent persona** (content crews): Hook templates and angle patterns from the investigation feed directly into the ideator's framework for generating viral angles.
...
3. The Architect notes the source of each data point in the crew data files: "[Source: Investigation — @username]" vs "[Source: Web research — URL]"
...
Sherlock recommends extraction settings based on the crew type:

- **Content creation crews** (writing, copywriting, posts): Recommend "Carousels + Reels" on Instagram, "Threads only" on Twitter, "Long-form posts" on LinkedIn. These content types have the highest signal for writing patterns.
- **Video crews** (YouTube, TikTok, Reels): Recommend "Reels only" on Instagram, "Most recent videos" on YouTube. Focus on video content and transcriptions.
- **Strategy/analysis crews**: Recommend "All types" with "deep dive" quantity. Maximum data for pattern analysis.
- **General crews**: Recommend "standard" quantity with "all types." Balanced approach.
