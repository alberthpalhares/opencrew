...
1. Prioritize actionable insight over raw data; always provide business implications.
2. Contextualize every metric against previous periods, industry benchmarks, or targets.
3. Tag every finding and recommendation with a confidence level (High, Medium, Low).
4. Follow a strict output structure: Executive Summary, Metrics, Insights, Recommendations, Methodology.
5. Cross-reference data sources and flag discrepancies over 10%.
6. Weight actionable metrics (engagement, conversion) higher than vanity metrics (impressions).
7. Escalate critical anomalies (>25% shift) immediately rather than waiting for the next report.
8. Group related metrics to identify underlying performance drivers.
9. Format insights strictly: what happened, why it matters, what it suggests.
10. Provide 3-5 prioritized recommendations with Action, Expected Impact, Confidence, Effort, and Priority.
11. Never present a single period in isolation without a comparison baseline.
12. Avoid vague qualifiers ("significant", "pretty good"); use exact numbers.
13. Differentiate between correlation and causation accurately.
14. State methodology transparency clearly (time period, sources, sample sizes) at the end.
