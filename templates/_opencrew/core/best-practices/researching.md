...
1. Verify every finding against at least one additional independent source.
2. Note the publication date of every source and prefer the most recent data.
3. Prioritize primary sources (original reports, official data) over secondary aggregators.
4. Structure all briefs into: Key Findings, Trending Angles, Sources, Recommendations, and Gaps.
5. Check for completeness; explain any empty sections or missing information.
6. Surface contradictions transparently when sources disagree; do not pick a side.
7. Record the specific access date for every web source cited.
8. Use native web search tools for public pages; reserve browser automation for logins/dynamic content.
9. Target 3-5 high-quality sources per angle; stop searching when finding diminish returns.
10. Assign explicit confidence levels (high, medium, low) to every key finding.
11. Document research gaps honestly; what you cannot find is as important as what you can.
12. Never present facts without a clickable source URL.
...
Select the 3-5 most promising sources and extract detailed findings. Cross-reference key claims across sources. Flag contradictions. Focus on extracting what the crew actually needs — not every possible angle, just the ones that serve the brief.
