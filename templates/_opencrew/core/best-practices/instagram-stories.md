...
1. Structure Stories in sequences of 3-7 vertical (9:16) frames that follow a narrative arc.
2. Grab attention in the first 2 seconds of the opener frame.
3. Keep text extremely short: maximum 2-3 lines of large, readable font per frame.
4. Ensure each frame can be consumed within 3-5 seconds without pausing.
5. Include at least one interactive element (poll, question, quiz, slider) per sequence.
6. Use specific, binary prompts for interactive elements rather than vague questions.
7. Maintain a casual, conversational, and immediate tone.
8. Add clear context text when using a link sticker to explain why the user should tap.
9. Avoid walls of text; break long messages across multiple frames.
10. Never repurpose horizontal content with black bars; always create vertical-native media.
11. Do not use hashtags or formal caption structures in Stories.
