...
1. Write subject lines between 30-50 characters to optimize for mobile truncation.
2. Draft custom preview text (60-90 chars) that extends, but never repeats, the subject line.
3. Keep total word count between 200-500 words to maintain click-through rates.
4. Place the key message and value proposition visible above the fold.
5. Use a single-column layout for mandatory mobile readability.
6. Include only one primary CTA button using action-oriented verb phrases.
7. Send consistently (weekly/biweekly) between 9-11 AM or 3-7 PM.
8. Break content into scannable sections with clear subheadings.
9. Use short paragraphs of 2-3 sentences maximum.
10. Personalize content beyond just the first name (segment, behavior).
11. Add descriptive alt text to all images in case they are blocked.
12. Avoid clickbait subject lines that do not deliver on their promise.
13. Include an unsubscribe link and maintain strict list hygiene.
