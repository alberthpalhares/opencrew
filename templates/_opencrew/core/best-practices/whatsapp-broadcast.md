...
1. Personalize the opening greeting using the `{{name}}` variable.
2. Hook the reader with a clear value proposition in the first two lines.
3. Keep the total message length between 300-500 characters.
4. Focus each message on a single purpose: value, promotion, or engagement.
5. Include only one clear, low-friction CTA (e.g., reply with a keyword, tap a link).
6. Limit emoji usage to 1-3 per message to maintain a professional, conversational tone.
7. Use WhatsApp formatting (*bold*, _italic_) to create visual hierarchy.
8. Always include a clear opt-out instruction (e.g., "Reply STOP to unsubscribe").
9. Send broadcasts only during recipient business hours (9 AM - 8 PM), avoiding weekends.
10. Restrict broadcast frequency to 2-4 messages per week to prevent high block rates.
11. Provide brief text context for any attached media (images, videos, or documents).
12. Ensure recipients have opted in and saved your number, or messages will not be delivered.
